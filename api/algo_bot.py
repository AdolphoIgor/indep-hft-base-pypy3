...
class Bot(Thread):
    ...
    @staticmethod
    def _create_position_order(basic_prov, thr, oms_provider) -> dict:
...
class AlgoBot(Bot):
    ...
    def _create_position(self, dct_start_cfg: dict, thr: dict) -> dict:
        ...
        def find_prev_sent_order(actual_msg: dict) -> list:
            ...
            lst_sent = []
            for sent in lst_orders.get("sent", []):
        ...
        while msg_ret is None:
            ...
            lst_orders = oms_provider.get("algo_msg_types", {}).get(self._algo_cfg.get("name"), [])
            if not lst_orders:
                continue

            lst_received = []
            for rec in lst_orders:
            ...
            for msg in lst_received:
                ...
                if rec.get("MsgType", "") == "8" and \
                        ((dct_msg.get("ClOrdID") == rec.get("ClOrdID")) or
                         (dct_msg.get("ClOrdID") == rec.get("OrigClOrdID"))):
                    ...
                    if rec.get("ExecType") == '8' and rec.get("OrdStatus") == '8' and \
                            rec.get("OrdRejReason") not in [4, 6, 7]:
                        self._shutdown = True

                    if not self._shutdown:
                        # if the order came back totally executed.
                        if rec.get("ExecType") in ['2', 'F'] and rec.get("OrdStatus") == '2':
                            msg_ret = rec
                            break

                        # if the order came back partially executed.
                        if rec.get("ExecType") in ['1', 'F'] and rec.get("OrdStatus") == '1':
                            ...
                            basic_prov.execute(**dtc_alt_order)
                        ...
                        if rec.get("ExecType") in lst_not_exec and rec.get("OrdStatus") in lst_not_exec:
                        ...
                        lst_prev_order = find_prev_sent_order(rec)

                        # if the order came back partially executed or totally executed.
                        if (rec.get("ExecType") in ['1', 'F'] and rec.get("OrdStatus") == '1') or \
                                (rec.get("ExecType") in ['2', 'F'] and rec.get("OrdStatus") == '2'):

                            # the original order was completelly executed and this one too
                            msg = lst_prev_order[-1]
                            if msg.get("MsgType") == "D" and msg.get('Side') != rec.get('Side') and \
                                    msg.get('OrderQty') == rec.get('OrderQty') and \
                                    msg.get('Price') == rec.get('Price'):
                                msg_ret = rec
                                break

                            dtc_alt_order = self._create_position_order(basic_prov, thr)
                            dtc_alt_order['Side'] = 2 if rec.get('Side') == 1 else 1
                            dtc_alt_order['OrderQty'] = rec.get("CumQty")
                        ...
                        if rec.get("ExecType") == '4' and rec.get("OrdStatus") == '4':
        ...
        self._set_thr_position(self._algo_cfg, oms_provider, thr, msg_ret)
